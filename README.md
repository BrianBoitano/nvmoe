...
**GLM-4.5-Air sits in between — and shows active params, not total params, set the ceiling.** Its four-workload trace (`traces/glm-all.tokens.jsonl`) measures top-10% experts at 26.7% of traffic and 37.0% token overlap — squarely between Qwen3 and V2-Lite. Cacheable enough, but at 12B *active* parameters each token needs ~4.2GB of experts, so even a decent hit rate leaves gigabytes per token on the SSD: measured 2.8 tok/s. Compare Qwen3-Next-80B: bigger name, same card, 44.8 tok/s — because only ~3B activate.
